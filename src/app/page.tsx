...
export default function GlobalMacroCommandCenter() {
  ...
  // Connect the global UI Zustand store securely
  ...
  return (
    <div className="relative w-full min-h-screen bg-slate-950 text-slate-100 overflow-hidden font-sans select-none">
      ...
      {/* 🛠️ MOBILE ACCORDION FIX: flex-col on mobile stacks the title and menu keys vertically so nothing overflows */}
      <header className="fixed top-4 left-4 right-4 z-40 flex flex-col md:flex-row bg-slate-950/85 backdrop-blur-md border border-slate-900 rounded-2xl p-4 gap-4 shadow-2xl max-w-7xl mx-auto transition-all">
        ...
        <nav className="grid grid-cols-1 sm:grid-cols-2 md:flex md:items-center flex-1 gap-2 md:justify-end w-full">
          ...
          <button 
            onClick={() => store.setActiveToggle?.('macro')}
            className={`flex items-center space-x-2 px-3 py-2.5 md:py-2 rounded-xl border text-xs font-mono font-medium transition-all ${
              store.activeToggle === 'macro'
          ...
          <button 
            onClick={() => store.setActiveToggle?.('inflation')}
            className={`flex items-center space-x-2 px-3 py-2.5 md:py-2 rounded-xl border text-xs font-mono font-medium transition-all ${
              store.activeToggle === 'inflation'
          ...
          {/* 🚀 THE HIDDEN LINK GATEWAY - Now perfectly wrapped and fully visible on all phone models */}
          <Link 
            href="/news"
      ...
      {isPreCaching && (
        <div className="fixed top-44 sm:top-24 left-4 z-30 animate-pulse">
      ...
      {/* Structural padding cushioning drops the interactive world SVG landmass grid cleanly below stacked menus */}
      <main className="w-full min-h-screen pt-52 sm:pt-28 pb-6 flex items-center justify-center relative z-10 px-4">
        <div className="w-full max-w-7xl h-[55vh] sm:h-[70vh] flex items-center justify-center relative">
          <EconomicMap />
        </div>
      </main>
